...
function updateGreeting() {
  ...
  if (h<6) {
    ...
    var greeting = "It's 5 o'clock somewhere, just not here.";
  ...
  if (month ==  12) {
    ...
    var holiday = "New Year's Resolution: 4K and 60FPS!!!";
  }
  else {
    var holiday = "";
  }
...
//disabling btc update functionality for now,
//the xhr request was slowing down the load time
// async function updateBTC() {
//   while (true) {
//     // var xhr = new XMLHttpRequest();
//     // xhr.open("GET", "https://api.coinbase.com/v2/prices/spot?currency=USD", false);
//     // xhr.send();
//     // end garbage fire, upgrading to Ajax soon
//
//     var obj = JSON.parse(xhr.response);
//
//     var btc = obj.data['amount'];
//     var yourBTC = btc * .10134975;
...
//
//
// 		 $("#btc").html("<h2>1 BTC = $" + btc + "<br>.1014 BTC =  $" + yourBTC + "</h2>");
//     if (!btcOld) {
...
function qodLoad() {
  $.ajax({
    ...
    dataType: 'json',
    success: function(a){
      var strQod = a["quote"]["body"];
      var strAuth = a["quote"]["author"];
      $("#qod").html("<a>" + strQod + "</a><br><a>- " + strAuth + "</a>");
  }});
}
