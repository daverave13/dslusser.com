...
function generate() {
  ...
  if (plant == "") {
    ...
    for (i; i<= n; i++) {
      ...
      strYardZoneSlot = "Insert into YARD_ZONE_SLOT (YARD_ID,YARD_ZONE_ID,YARD_ZONE_SLOT_ID,YARD_ZONE_SLOT_NAME,YARD_ZONE_SLOT_STATUS,X_COORDINATE,Y_COORDINATE,Z_COORDINATE,MAX_CAPACITY,USED_CAPACITY,MARK_FOR_DELETION,IS_GUARD_HOUSE,IS_THRESHOLD_GUARD_HOUSE,CREATED_DTTM,LAST_UPDATED_DTTM,LOCN_ID,CREATED_SOURCE,CREATED_SOURCE_TYPE,LAST_UPDATED_SOURCE,LAST_UPDATED_SOURCE_TYPE) values (" + YID + "," + YZID + ",SEQ_YARD_ZONE_SLOT_ID.NEXTVAL,'" + prefix + i + "',204,0,0,0,1,0,0,0,0,to_timestamp('24-APR-18 10.51.38.000000000 AM','DD-MON-RR HH.MI.SSXFF AM'),to_timestamp('24-APR-18 10.51.38.000000000 AM','DD-MON-RR HH.MI.SSXFF AM'),(SELECT LOCN_ID FROM LOCN_HDR WHERE LOCN_BRCD = '" + prefix + i + "'),'DATALOAD',2,'DATALOAD',2);"
      strLocation = "INSERT INTO LOCATION (SELECT SEQ_LOCATION_ID.NEXTVAL, 32, 1, YARD_ID, YARD_ZONE_ID, YARD_ZONE_SLOT_ID, NULL, 0, YARD_ZONE_SLOT_ID, 0, 0, SYSDATE, SYSDATE FROM YARD_ZONE_SLOT WHERE yard_zone_slot_name = '" + prefix + i + "');"
      strWhseSysCode = "Insert into WHSE_SYS_CODE (REC_TYPE,CODE_TYPE,WHSE,CODE_ID,CODE_DESC,SHORT_DESC,MISC_FLAGS,CREATE_DATE_TIME,MOD_DATE_TIME,USER_ID,WHSE_SYS_CODE_ID,SYS_CODE_TYPE_ID,WM_VERSION_ID,CREATED_DTTM,LAST_UPDATED_DTTM) values ('C','SLT','" + plant + "','" + prefix + i + "','" + prefix + i + "','" + prefix + i + "','',to_date('09-DEC-17 06:09:17','DD-MON-RR HH24:MI:SS'),to_date('09-DEC-17 06:09:17','DD-MON-RR HH24:MI:SS'),null,WHSE_SYS_CODE_ID_SEQ.nextval,null,1,to_timestamp('09-DEC-17 06.09.17.000000000 AM','DD-MON-RR HH.MI.SSXFF AM'),to_timestamp('19-APR-18 08.59.15.322137000 AM','DD-MON-RR HH.MI.SSXFF AM'));"
      $("#genTarget").append(strLocationHDR + "<br><br>").append(strYardZoneSlot + "<br><br>").append(strLocation + "<br><br>");
    }
  }
}
